                ...
                with tab2:
                    ...
                    with st.expander('Click to view pie plots for Country and Year-wise sale count:', expanded = True):
                        ...
                        with subcol2:
                            ...
                            st.markdown(f"""
                                        #### Insights
                                        - {res.iloc[0,0]} dominates with {res.iloc[0,1]/total*100:.2f}% of records, suggesting it was a peak year or more comprehensive data collection
                                        - {res.iloc[1,0]} follows with {res.iloc[1,1]/total*100:.2f}%, while {res.iloc[2,0]} and {res.iloc[3,0]} each represent about {res.iloc[2,1]/total*100:.0f}%
                                        - The distribution may reflect business growth in {res.iloc[0,0]} or data collection methodology differences
                                        - Recent years ({res.iloc[2,0]}-{res.iloc[3,0]}) show consistent but lower representation than {res.iloc[0,0]}
                                        """)
